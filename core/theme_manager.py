...
class ThemeManager:
    ...
    def get_icon_url(icon_name: str) -> str:
        """Получить file:// URL к иконке (для Qt stylesheets на Windows важны кавычки/URI)"""
        icon_path = Path(__file__).parent / "icons" / icon_name
        return icon_path.absolute().as_uri()
    ...
    /* Скрываем стандартные индикаторы/линии там, где Qt их рисует через border-image */
    ...
    /* Важно: на Windows путь вида C:/... в url(...) может не примениться без кавычек.
       Поэтому в шаблоне url("...") и подставляется file:///... */
    ...
    def apply_theme(theme_name: str):
        ...
        if theme_name == "dark":
            ...
            stylesheet = ThemeManager.DARK_STYLESHEET_TEMPLATE.format(
                branch_closed_icon=ThemeManager.get_icon_url("branch-closed.png"),
                branch_open_icon=ThemeManager.get_icon_url("branch-open.png"),
            )
